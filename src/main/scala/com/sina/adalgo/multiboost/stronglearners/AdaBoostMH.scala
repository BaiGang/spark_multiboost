...
class AdaBoostMHModel[BM <: MultiLabelClassificationModel](
  ...
  override def predict(testData: Vector): Vector = {
    baseLearners.map{ case baseLearner =>
...
@Experimental
class AdaBoostMHAlgorithm[
    BM <: BaseLearnerModel,
    BA <: BaseLearnerAlgorithm[BM]](
    baseLearnerAlgo: BA,
    _numClasses: Int,
...
object AdaBoostMH {
  ...
  def train(
    ...
    val adaboostMHAlgo = new AdaBoostMHAlgorithm[
        DecisionStumpModel, DecisionStumpAlgorithm](
        decisionStumpAlgo,
        numClasses,
